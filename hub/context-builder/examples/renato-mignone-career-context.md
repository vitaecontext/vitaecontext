...
Notes: MIT-licensed open-source career-context system and modular agent skill toolkit. VitaeContext gives AI agents a private, reusable source of truth about a person's career, then provides focused skills for turning that context into grounded professional work. Its Career Context artifact stores verified facts, goals, links, evidence, constraints, and claims to avoid; VitaeGraph is the optional deeper structured-record layer. The npm/Node CLI distributes the shared methodology into provider-specific skill layouts so agents can reuse grounded context across CVs, profiles, portfolios, applications, interviews, and related professional work.
