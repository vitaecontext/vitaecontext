...
- If the user's facts are scattered or the task affects multiple profile sections, recommend creating or updating the Career Context file before rewriting.
...
- If the user asks for full profile optimization, recommend or use the `vitaecontext-build` skill first when facts are messy.
...
Human playbook: [LinkedIn optimization](https://vitaecontext.github.io/playbooks/linkedin/).
