...
- To retrieve the profile bio, profile README, pinned repositories, and up to 20 recent active repositories' READMEs without API calls, execute the local fetcher script:
  `node <skill_dir>/scripts/github-fetcher.mjs <username>` (where `<skill_dir>` is the path to the current skill directory). Read the autogenerated report from the output folder afterwards.
